...
describe "#starts_with_consonant?" do
  ...
  it 'classifies true cases [10 points]' do
    expect(starts_with_consonant?('v')).to be_true, "'v' is a consonant"
    ['v', 'vest', 'Veeee', 'crypt'].each do |string|
      expect(starts_with_consonant?(string)).to be_true, "Incorrect results for input: \"#{string}\""
    end
  end
  it 'classifies false cases [10 points]' do
    expect(starts_with_consonant?('a')).to be_false, "'a' is not a consonant"
    ['asdfgh', 'Unix'].each do |string|
      expect(starts_with_consonant?(string)).to be_false, "Incorrect results for input: \"#{string}\""
    end
  end
  it 'works on the empty string [5 points]' do
    expect(starts_with_consonant?('')).to be_false
  end
  it 'works on nonletters [5 points]' do
    expect(starts_with_consonant?('#foo')).to be_false
  end
end
...
describe "#binary_multiple_of_4?" do
  ...
  it "classifies valid binary numbers [30 points]" do
    ["1010101010100", "0101010101010100", "100", "0"].each do |string|
      expect(binary_multiple_of_4?(string)).to be_true,  "Incorrect results for input: \"#{string}\""
    end
    ["101", "1000000000001"].each do |string|
      expect(binary_multiple_of_4?(string)).not_to be_true,  "Incorrect results for input: \"#{string}\""
    end
  end
  it "rejects invalid binary numbers [10 points]" do
    expect(binary_multiple_of_4?('a100')).to be_false, "'a100' is not a valid binary number!"
    expect(binary_multiple_of_4?('')).to be_false, "The empty string is not a valid binary number!"
  end
end