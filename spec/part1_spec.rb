describe 'Ruby intro part 1' do
  ...
  describe "#sum_to_n" do
    ...
    it "returns true when any two elements sum to the second argument [30 points]" do
      expect(sum_to_n?([1,2,3,4,5], 5)).to be_truthy
      expect(sum_to_n?([3,0,5], 5)).to be_truthy
      expect(sum_to_n?([-1,-2,3,4,5,-8], 12)).to be_falsey
      expect(sum_to_n?([-1,-2,3,4,6,-8], 12)).to be_falsey
    end
    #    for rspec 2.14.1
    ...
    it "returns false for the single element array [5 points]" do
      ...
      expect(sum_to_n?([3], 0)).to be_falsey
    end
    it "returns false for the empty array [5 points]" do
      expect(sum_to_n?([], 0)).to be_falsey
      expect(sum_to_n?([], 7)).to be_falsey
    end
  end
end